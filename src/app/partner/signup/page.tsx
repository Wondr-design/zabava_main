...
function SignupInner() {
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <form onSubmit={onSubmit} className="w-full max-w-md space-y-4 border rounded-xl p-6 bg-white/5">
        <h1 className="text-xl font-semibold">Partner Signup</h1>
        {error && <p className="text-red-500 text-sm">{error}</p>}
        <div className="space-y-2">
          <label className="block text-sm">Invite token</label>
          <input className="w-full rounded border px-3 py-2 bg-white/80 text-black" value={token} onChange={(e) => setToken(e.target.value)} />
        </div>
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="space-y-2">
            ...
            <input className="w-full rounded border px-3 py-2 bg-white/80 text-black" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <input className="w-full rounded border px-3 py-2 bg-white/80 text-black" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
          </div>
        </div>
