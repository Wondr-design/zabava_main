...
function isAuthorized(req: NextRequest) {
  ...
  try {
    ...
    log.warn("admin_deal_analytics_auth_failed", {
...
export async function GET(
  ...
  if (!id) {
    return withCors(NextResponse.json({ error: "Deal ID is required" }, { status: 400 }), CORS_CONFIG);
  }
  ...
  try {
    ...
    log.error("admin_deal_analytics_error", error, {
      route: "admin/deals/[id]/analytics",
      correlationId: getCorrelationId(req),
      dealId: id,
    });
    return withCors(
