...
import {
  listDueDealReminders,
  updateDealReminderStatuses,
...
function isAuthorized(req: NextRequest) {
  ...
  try {
    ...
    log.warn("admin_deal_reminder_dispatch_auth_failed", {
...
export async function GET(req: NextRequest) {
  ...
  try {
    const reminders = await listDueDealReminders();
    const response = NextResponse.json({ reminders });
    response.headers.set("x-csrf-token", generateCsrfToken());
    return withCors(response, CORS_CONFIG);
  } catch (error) {
    log.error("admin_deal_reminder_list_error", error, {
      correlationId: getCorrelationId(req),
    });
    return withCors(
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await updateDealReminderStatuses(parsed.data.reminders);
    const response = NextResponse.json({ ok: true, ...result });
    response.headers.set("x-csrf-token", generateCsrfToken());
    return withCors(response, CORS_CONFIG);
  } catch (error) {
    log.error("admin_deal_reminder_update_error", error, {
      correlationId: getCorrelationId(req),
    });
    return withCors(
      NextResponse.json({ error: "Internal server error" }, { status: 500 }),
