...
export function DealGenerateForm({
  ...
  const groupedRequirements = useMemo(() => {
    ...
    >();
    ticketRequirements.forEach((item) => {
    ...
    return Array.from(map.values()).map((entry, index) => {
      const totalQuantity = entry.subItems.reduce(
        (sum, sub) => sum + (sub.quantity || 0),
        0
      );
      return {
        key: `${entry.ticketType.toLowerCase()}::${index}`,
  ...
  useEffect(() => {
    ...
    setSelectedTicketKey((current) => {
      if (current && groupedRequirements.some((req) => req.key === current)) {
        return current;
      }
      return groupedRequirements[0]?.key ?? null;
    });
  }, [ticketRequirements, groupedRequirements, minVisitors]);

  const selectedRequirement = useMemo(
    () =>
      groupedRequirements.find((req) => req.key === selectedTicketKey) ?? null,
    [groupedRequirements, selectedTicketKey]
  );

  useEffect(() => {
    if (selectedRequirement) {
      setVisitors(selectedRequirement.totalQuantity || minVisitors);
  ...
  const ticketBreakdown = useMemo(() => {
    ...
    // Include all sub-items for the selected ticket type; sub-options are informational, not separate ticket types.
    const items = selectedRequirement.subItems.length
      ? selectedRequirement.subItems
      : [
          {
            subType: null,
            quantity: selectedRequirement.totalQuantity || minVisitors,
          },
        ];
  ...
  }, [selectedRequirement, minVisitors]);
    ...
    <div className="space-y-6 rounded-3xl border border-white/10 bg-white/5 p-6">
      <header className="space-y-2">
        <h2 className="text-lg font-semibold text-white">
          Generate QR reservation
        </h2>
        <p className="text-sm text-slate-300">
          We email the QR pass and hold a slot with {partnerName ?? "the venue"}
          . QR codes expire
          {qrValidityDays === 1
            ? " after 1 day"
            : ` after ${qrValidityDays} days`}{" "}
          and staff will decline entry if fewer than {minVisitors} visitors
          arrive.
        </p>
        {timeZoneLabel ? (
          <p className="text-xs text-slate-400">
            Valid time zone: {timeZoneLabel}
          </p>
        ) : null}
      </header>
      ...
      {!isActive && (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
          <p>
            This deal is not currently active. You can browse other specials or
            contact us at{" "}
            <a
            ...
            <section className="space-y-5 rounded-3xl border border-white/10 bg-white/5 p-5 text-sm text-slate-200">
              <div className="space-y-1">
                ...
                <h3 className="text-lg font-semibold text-white">
                  Choose the ticket type for your group
                </h3>
                <p className="text-xs text-slate-300">
                  Each ticket type requires a specific headcount. Select the one
                  that matches your group and confirm the size.
                </p>
              </div>
                ...
                {groupedRequirements.map((requirement) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-slate-300 flex flex-col gap-1">
                        <span>
                          Requires{" "}
              ...
              <label className="flex items-start gap-3 text-xs text-slate-200">
                <input
                  ...
                  onChange={(event) =>
                    setConfirmedRequirement(event.target.checked)
                  }
                ...
                <span>
                  ...
                  <span className="font-semibold text-white">
                    {selectedRequirement?.totalQuantity ?? "—"}
                  </span>{" "}
                  guest
                  {(selectedRequirement?.totalQuantity ?? 0) === 1 ? "" : "s"}{" "}
                  for the selected ticket type.
                </span>
              </label>
              {error ? (
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!confirmedRequirement) {
                      setError(
                        "Please confirm your group size matches the requirement."
                      );
                      return;
                    }
                    setError(null);
              ...
              onSubmit={async (event) => {
                ...
                try {
                  const response = await fetch(
                    `/api/public/special-deals/${slug}/generate`,
                    {
                      method: "POST",
                      ...
                      body: JSON.stringify({
                        email,
                        visitors,
                        ticketBreakdown: ticketRequirements.length
                          ? ticketBreakdown
                          : undefined,
                        consentMarketing,
                  ...
                  console.error("public_deal_generate_submit_error", err);
                  const message =
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                {ticketRequirements.length > 0 && selectedRequirement ? (
                  <div className="flex flex-col gap-1 rounded-2xl border border-white/10 bg-slate-900/40 px-4 py-3">
                    ...
                    <p className="text-xs text-slate-300">
                      {selectedRequirement.totalQuantity} guest
                      {selectedRequirement.totalQuantity === 1 ? "" : "s"}{" "}
                      required · visitors auto-set to{" "}
                      {selectedRequirement.totalQuantity}.
                    </p>
                  </div>
                ) : (
                  <label className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      value={visitors}
                      onChange={(event) =>
                        setVisitors(Number(event.target.value))
                      }
                      className="rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white shadow-inner focus:border-indigo-400 focus:outline-none"
                    />
                    <span className="text-[11px] text-slate-400">
                      Minimum {minVisitors} visitor
                      {minVisitors === 1 ? "" : "s"} required for {title}.
                    </span>
                  </label>
                )}
              </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-200">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-300">
                        We&apos;ll validate {selectedRequirement.totalQuantity}{" "}
                        guest
                        {selectedRequirement.totalQuantity === 1 ? "" : "s"} for
                        this ticket type at check-in.
                      </p>
                    </div>
                    <Button
                      type="button"
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          setVerificationMessage(
                            "Email verified. You can now generate the QR pass."
                          );
                        } catch (err) {
                          const message =
                            err instanceof Error
        ...
        >
          <dl className="mt-3 space-y-2 text-xs text-indigo-200">
            <div className="flex items-center gap-2 text-emerald-200">
              <dt className="w-28 uppercase tracking-wide text-emerald-300">
                Visit ID
              </dt>
            ...
            {result.staffScanUrl ? (
              <div className="flex items-center gap-2 text-emerald-200">
                ...
                <dd className="font-mono break-all text-emerald-100">
                  {result.staffScanUrl}
                </dd>
              </div>
            ) : null}
