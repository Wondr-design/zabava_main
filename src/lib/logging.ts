...
export const log = {
  ...
  error: (message: string, err?: unknown, context?: Record<string, unknown>) => emit('error', message, { error: safeError(err), ...(context || {}) }),
};

export function getCorrelationId(req?: { headers?: Headers | Record<string, unknown> } | null) {
  try {
  ...
  try {
    ...
    const lower = Object.fromEntries(Object.entries(h2).map(([k, v]) => [k.toLowerCase(), String(v)]));
