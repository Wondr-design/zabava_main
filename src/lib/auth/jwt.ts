...
type SignOptionsCompat = Omit<SignOptions, 'expiresIn'> & { expiresIn?: string | number };

export function signJwt(payload: object, options?: SignOptionsCompat) {
  const secret = getSecret();
  const { expiresIn, ...rest } = options || {};
  const merged: SignOptions = {
    expiresIn: (expiresIn ?? getDefaultExpiresIn()) as unknown as import('ms').StringValue | number,
    ...rest,
  };
  return jwt.sign(payload, secret, merged);
}

export function verifyJwt<T extends object = Record<string, unknown>>(token: string, options?: VerifyOptions) {
  const secret = getSecret();
